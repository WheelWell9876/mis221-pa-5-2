...
static bool ExistingCustomerMenu(Movie[] myMovies, Payment[] myPayments, Person[] myPersons, MovieUtility movieUtility, PaymentUtility paymentUtility, PersonUtility personUtility,  MovieReports movieReports, PaymentReports paymentReports, PersonReports personReports, FileItem file)
{
    ...
    if(SelectEmail == true){
        ...
        {
            System.Console.WriteLine(" ");
            System.Console.WriteLine("Welcome customer. What would you like to do?");
            ...
            while(CustomerOption == true)
            {
                switch(Console.ReadLine())
                {
                    ...
                    case "9":
                        ...
                        break;
                    case "10":
                        ReturnMovie(myPayments, paymentUtility);
                        CustomerOption = true;
                        break;
                    case "11":
...
static bool AccessReports(Movie[] myMovies, Payment[] myPayments, Person[] myPersons, MovieUtility movieUtility, PaymentUtility paymentUtility, PersonUtility personUtility, MovieReports movieReports, PaymentReports paymentReports, PersonReports personReports, FileItem file)
{
    ...
    while(AdminOption == true)
    {
        ...
        System.Console.WriteLine("11. Top five movies rented per genre.");
        ...
        switch(Console.ReadLine())
        {
            ...
            case "12":
                ...
                break;
            case "13":
                paymentReports.TotalPayments();
                AdminOption = true;
                break;
            case "14":
                paymentReports.TotalRevenue();
                AdminOption = true;
                break;
            case "15":
...
static void RentMovie(Movie[] myMovies, Payment[] myPayments, MovieUtility movieUtility, MovieReports movieReports, FileItem file)
{
    ...
    int selectedMovie = Payment.GetCount();
    ...
    if(index != -1)
    {
        ...
        newPayment.SetTitle(myMovies[index].GetTitle());
        newPayment.SetEmail(myPayments[index].GetEmail());
        newPayment.SetMovieID(myMovies[index].GetMovieID());
        newPayment.SetGenre(myMovies[index].GetGenre());
        newPayment.SetBuy(DateTime.Today); //gets current date for later usage
        newPayment.SetReturn(DateTime.Today);
...
static void ReturnMovie(Payment[] myPayments, PaymentUtility paymentUtility)
{
    System.Console.WriteLine("Enter your email.");
    string userEmail = Console.ReadLine();
    // string index = paymentUtility.FindPayments(userEmail);
...
static void AddPerson(Person[] myPersons, PersonUtility personUtility, PersonReports personReports, FileItem file)
{
    ...
    file.GetAllPersons();
    personReports.PrintAllPersons(myPersons, file);
    System.Console.WriteLine("Pick an ID for your new username. It must be a postive integer and an ID that is not currently listed above.");
